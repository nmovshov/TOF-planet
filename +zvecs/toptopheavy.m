...
%    zvec = TOPTOPHEAVY(N) returns a vector of normalized radii with two thirds of
%    them distributed in the top half and the rest equally distributed in the
...
%
%    zvec = TOPTOPHEAVY(N, skew) returns a vector of normalized radii with skew(1)
%    of them distributed in the interval [1-skew(2),1] and the rest equally
%    distributed in the interval (0,1-skew(2)]. The upper distribution follow the
%    same rule: skew(1) of those layers in the interval [1-skew(2)^2,1] and the
%    rest in the interval [1-skew(2),1-skew(2)^2].
