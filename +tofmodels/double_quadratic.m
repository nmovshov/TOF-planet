function tof = double_quadratic(N, x, zstrat, forcemono)
%DOUBLE_QUADRATIC Piecewise-quadratic density profile.
%    DOUBLE_QUADRATIC(N, x) returns an N-point TOFPlanet object with rhoi
%    approximated by a piecewise-quadratic function:
%    
%        rho = x(1)*s^2 + x(2)*s + x(3), if s > x(6),
%        rho = x(4)*s^2 + x(5)*s + 1,    otherwise.
%    
%    The default level spacing is one of equal radius increments between s/s0=1
%    and s/s0=1/N.
%    
%    DOUBLE_QUADRATIC(N, x, zstrat) lets you specify the zvec distribution
...
%
%    DOUBLE_QUADRATIC(...,forcemono) where forcemono==true forces the resulting
%    density profile to be monotonically nonincreasing. Default forcemono is true.

if nargin == 0 && nargout == 0
    help('tofmodels.double_quadratic')
    return
end
narginchk(2,4)
...
validateattributes(x,{'numeric'},{'real','vector','numel',6},2)
validateattributes(zstrat,{'function_handle'},{},'','zstrat',3)
validateattributes(forcemono,{'logical'},{'scalar'},'','forcemono',4)
assert(x(6)>0 && x(6)<1, 'Transition (normalized) radius must be in (0,1).')
