function tof = triple_quadratic(N, x, zstrat, forcemono)
%TRIPLE_QUADRATIC Piecewise-quadratic density profile.
%    TRIPLE_QUADRATIC(N, x) returns an N-point TOFPlanet object with rhoi
%    approximated by a piecewise-quadratic function:
%    
%        rho = x(1)*s^2 + x(2)*s + x(3), if s > x(9),
%        rho = x(4)*s^2 + x(5)*s + x(6), if x(10) < s < x(9),
%        rho = x(7)*s^2 + x(8)*s + 1,    otherwise.
%    
%    The default level spacing is one of equal radius increments between s/s0=1
%    and s/s0=1/N.
%    
%    TRIPLE_QUADRATIC(N, x, zstrat) lets you specify the zvec distribution
%    strategy. Pass a handle to a function that takes a single scalar integer
%    (number of layers) and returns a vector of that length with values in the
%    interval (0, 1], for normalized mean level radii.
%
%    TRIPLE_QUADRATIC(...,forcemono) where forcemono==true forces the resulting
%    density profile to be monotonically nonincreasing. Default forcemono is true.

if nargin == 0 && nargout == 0
    help('tofmodels.triple_quadratic')
    return
end
narginchk(2,4)
if ((nargin < 3) || isempty(zstrat)), zstrat = @zvecs.best; end
if ((nargin < 4) || isempty(forcemono)), forcemono = true; end
validateattributes(N,{'numeric'},{'positive','integer'},'','N',1)
validateattributes(x,{'numeric'},{'real','vector','numel',10},2)
validateattributes(zstrat,{'function_handle'},{},'','zstrat',3)
validateattributes(forcemono,{'logical'},{'scalar'},'','forcemono',4)
assert(x(9)>0 && x(9)<1, 'Transition (normalized) radius must be in (0,1).')
assert(x(10)>0 && x(10)<1, 'Second transition radius must be in (0,1).')
assert(x(10) <= x(9), 'Second transition must come before first transition.')

tof = TOFPlanet();

zvec = zstrat(N);
assert(isnumeric(zvec) && isvector(zvec) && (numel(zvec) == N),...
    '@zstrat(N) must return a vector of length N with values in (0,1].')
assert(all(zvec > 0) && all(zvec <= 1),...
    '@zstrat(N) must return a vector of length N with values in (0,1].')

a1 = x(1); b1 = x(2); c1 = x(3);
a2 = x(4); b2 = x(5); c2 = x(6);
a3 = x(7); b3 = x(8); c3 = 1;
zt = x(9);
zc = x(10);

dvec = NaN(1,N);
for k=1:N
    if zvec(k) > zt
        dvec(k) = a1*zvec(k)^2 + b1*zvec(k) + c1;
    elseif zvec(k) > zc
        dvec(k) = a2*zvec(k)^2 + b2*zvec(k) + c2;
    else
        dvec(k) = a3*zvec(k)^2 + b3*zvec(k) + c3;
    end
end

if forcemono
    dvec(1) = max(dvec(1), 0);
    for k=2:N
        dvec(k) = max(dvec(k), dvec(k-1));
    end
end

tof.si = zvec;
tof.rhoi = dvec;

end
