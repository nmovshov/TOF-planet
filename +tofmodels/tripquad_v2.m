function tof = tripquad_v2(N, x, forcemono, zstrat)
%TRIPQUAD_V2 Piecewise-quadratic density profile.
%    TRIPQUAD_V2(N, x) returns an N-point TOFPlanet object with normalized rhoi
...
%    end points and a curvature coefficient. The segment breakpoints are at z1
%    (upper) and z2 (lower). The density at z1 is d11 (the right-limit) and d21
%    (the left-limit). The density at z2 is d22 (the right-limit) and d32 (the
...
%        y21: log(d21 - d11) - log(1 - d21)
%        y22: log(d22 - d21) - log(1 - d22)
%        a3: curvature of third segment (a3*x^2 + b3*x + c3)
%        y32: log(d32 - d22) - log(1 - d32)
%        lz1: log(z1) - log(1 - z1)
...
%    z=1/N.
%    
%    TRIPQUAD_V2(...,forcemono) where forcemono==true forces the resulting density
%    profile to be monotonically nonincreasing. Default forcemono is false.

if nargin == 0 && nargout == 0
    help('tofmodels.tripquad_v2')
    return
end
narginchk(2,4)
...
d21 = (d11 + bexp(y21))/(1 + bexp(y21));
d22 = (d21 + bexp(y22))/(1 + bexp(y22));
d32 = (d22 + bexp(y32))/(1 + bexp(y32));
